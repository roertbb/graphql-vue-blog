...
export default async (event: FunctionEvent<EventData>) => {
  ...
  try {
    ...
    const { email, nick, password } = event.data;
    ...
    // create new user
    const userId = await createGraphcoolUser(api, email, nick, hash);

    // generate node token for new User node
    const token = await graphcool.generateNodeToken(userId, 'User');

    return { data: { id: userId, nick, token } };
  } catch (e) {
    console.log(e);
...
async function createGraphcoolUser(
  ...
  const mutation = `
    mutation createGraphcoolUser($email: String!, $nick: String!, $password: String!) {
      createUser(
        email: $email,
        nick: $nick,
        password: $password
